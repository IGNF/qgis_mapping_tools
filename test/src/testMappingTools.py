...
class TestMappingTools(object):
    ...
    def findButtonByActionName(self, buttonActionName):
        '''Find button corresponding to the given action.
            ...
            :rtype: QWidget or None
        '''
        for tbar in iface.mainWindow().findChildren(QtWidgets.QToolBar):
            for action in tbar.actions():
                if action.text() == buttonActionName:
                    for widget in action.associatedWidgets():
                        if type(widget) == QtWidgets.QToolButton:
                            return widget
        return None
    ...
    def mapToScreenPoint(self, mapCoordPoint):
        ...
        sourceScreen = iface.mapCanvas().getCoordinateTransform().transform(QgsPointXY(mapCoordPoint))
    ...
    def testFusion(self):
        ...
        self.dragAndDropMap(QgsPointXY(783902,6528458),  QgsPointXY(785223,6528279))
        QgsApplication.processEvents()
