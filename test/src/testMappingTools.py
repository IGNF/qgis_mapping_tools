...
class TestMappingTools(object):
    ...
    TESTS_TO_CHECK = 'up_to_undo' # available values : 'up_to_fusion', 'up_to_undo', 'up_to_quit_edit' , every thing else for full test
    ...
    def printTest(self, expected, result, testName, valueToTestName):
        '''Print test result.
            ...
            :type expected: all types possible
            :param result: result value to test.
            :type result: all types possible
    ...
    def testFusion(self):
        ...
        # Simulate click on plugin button fusion
        self.clickOn('Fusion')
        ...
        if not self.printTest(True, self.findButtonByActionName(oldMapToolActionName).isChecked(), 'Check previous map tool button ('+oldMapToolActionName+')', 'Previous map tool button ('+oldMapToolActionName+') status'):
            return
